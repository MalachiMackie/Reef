...
public partial class ProgramAbseil
{
    ...
    private IExpressionResult LowerExpression(IExpression expression, IPlace? destination)
    {
        ...
        return expression switch
        {
            ...
            CollectionExpression collectionExpression => LowerCollectionExpression(collectionExpression, destination),
            FillCollectionExpression fillCollectionExpression => LowerFillCollectionExpression(fillCollectionExpression, destination),
            IndexExpression indexExpression => LowerIndexExpression(indexExpression, destination),
            _ => throw new ArgumentOutOfRangeException(nameof(expression))
        };
    }

    private IExpressionResult LowerIndexExpression(
        IndexExpression indexExpression, IPlace? destination)
    {
        ...
        var collectionType = GetTypeReference(indexExpression.Collection.ResolvedType.NotNull());
        var arrayType = collectionType switch
        {
            LoweredArray x => x,
        ...
        else if (collectionResult is OperandResult(var operand))
        {
            var localName = LocalName((uint)_locals.Count);
            _locals.Add(new MethodLocal(localName, null, collectionType));
            collectionPlace = new Local(localName);
            _basicBlockStatements.Add(
                new Assign(collectionPlace, new Use(operand)));
        }
        else
        {
    ...
    private IExpressionResult LowerFillCollectionExpression(
        FillCollectionExpression fillCollectionExpression, IPlace? destination)
    {
        var type = GetTypeReference(fillCollectionExpression.ResolvedType.NotNull());
    ...
    private IExpressionResult LowerCollectionExpression(CollectionExpression collectionExpression, IPlace? destination)
    {
        ...
        {
            LowerExpression(collectionExpression.Elements[(int)i], new Index(newDestination, new UIntConstant(i, 8)));
        }

        return new PlaceResult(destination);
    }

    private (LoweredArray, IPlace) CreateArray(ILoweredTypeReference type, IPlace destination)
    {
        if (type is LoweredPointer(LoweredConcreteTypeReference pointerTo))
        {
            ...
            type = pointerTo.TypeArguments[0];

            _basicBlockStatements.AddRange([
                new Assign(
                    new Deref(destination),
                    new CreateObject(BoxedValueType(type))
                ),
                new Assign(
            ...
            destination = new Field(new Deref(destination), "Value", ClassVariantName);
        }

        if (type is not LoweredArray arrayType)
        {
            throw new InvalidOperationException($"Expected array type, but found {type}");
        }
    ...
    private IExpressionResult LowerIf(IfExpressionExpression expression, IPlace? destination)
    {
        ...
        bodyBasicBlockId.Id = GetNextEmptyBasicBlock().Id;
        LowerExpression(expression.IfExpression.Body.NotNull(), destination);
        ...
        foreach (var (i, elseIf) in expression.IfExpression.ElseIfs.Index())
        {
            ...
            LowerExpression(elseIf.Body.NotNull(), destination);
        }
        ...
        if (expression.IfExpression.ElseBody is not null)
        {
            ...
            LowerExpression(expression.IfExpression.ElseBody, destination);
        }
            ...
            foreach (var variantNode in variantNodes)
            {
                ...
                if (variantNode.Expression is not null)
                {
                    ...
                    else
                    {
                        ...
                        LowerExpression(variantNode.Expression.NotNull(), destination);
                    }
                }
            ...
            case ClassPattern classPattern:
                {
                    ...
                    if (valueType is LoweredPointer && accessorPlace is not Field(Deref, _, _))
                    {
                        accessorPlace = new Field(new Deref(accessorPlace), "Value", ClassVariantName);
                    }
    ...
    private IExpressionResult LowerMatches(
        ...
        IPlace? destination)
    {
        var valueResult = LowerExpression(e.ValueExpression, destination: null);
        ...
        IPlace? destination)
    {
        ...
        {
            case LoweredConcreteTypeReference unboxedTypeReference:
                definitionId = unboxedTypeReference.DefinitionId;
                break;
            case LoweredPointer(LoweredConcreteTypeReference boxedValue):
                {
                    Debug.Assert(boxedValue.DefinitionId == DefId.BoxedValue);
                    definitionId = (boxedValue.TypeArguments[0] as LoweredConcreteTypeReference).NotNull().DefinitionId;
                    break;
                }
            default:
    ...
    private PlaceResult LowerStaticMemberAccess(
            ...
            case MemberType.Variant:
                {
                    ...
                    LoweredConcreteTypeReference concreteType;
                    switch (unionType)
                    {
                        case LoweredPointer(LoweredConcreteTypeReference pointerTo):
                            {
                                Debug.Assert(pointerTo.DefinitionId == DefId.BoxedValue);
                                concreteType = (pointerTo.TypeArguments[0] as LoweredConcreteTypeReference).NotNull();
                                break;
                            }
                        case LoweredConcreteTypeReference unboxedTypeReference:
                            concreteType = unboxedTypeReference;
                            break;
                        default:
                            throw new InvalidOperationException();
                    }
    ...
    private PlaceResult CreateObject(
        ILoweredTypeReference type,
        string variantName,
        ...
        IPlace? destination)
    {
        ...
        LoweredConcreteTypeReference concreteType;
        LoweredConcreteTypeReference? boxedValueType = null;
        switch (type)
        {
            case LoweredPointer(LoweredConcreteTypeReference pointerTo):
                {
                    Debug.Assert(pointerTo.DefinitionId == DefId.BoxedValue);
                    boxedValueType = pointerTo;
                    concreteType = (pointerTo.TypeArguments[0] as LoweredConcreteTypeReference).NotNull();
                    break;
                }
            case LoweredConcreteTypeReference x:
                {
                    concreteType = x;
                    break;
                }
        ...
        var isBoxed = boxedValueType is not null;
        ...
        if (isBoxed)
        {
            _basicBlocks[^1].Terminator = new MethodCall(
                new LoweredFunctionReference(DefId.Allocate, []),
                [new SizeOf(boxedValueType.NotNull())],
                destination,
                new BasicBlockId($"bb{_basicBlocks.Count}"));
            ...
            CreateObject(
                ...
                ClassVariantName,
                // fields are empty because we create the object into them below
                fields: [],
                destination);
            ...
            _basicBlockStatements.Add(new Assign(
                new Field(new Field(destination, "ObjectHeader", ClassVariantName), "TypeId", ClassVariantName),
                new Use(new TypeIdOf(concreteType))
            ));

            destination = new Field(destination, "Value", ClassVariantName);
        }
        ...
        return new PlaceResult(returnValue);
    }
    ...
    private IExpressionResult LowerTuple(TupleExpression tupleExpression, IPlace? destination)
    {
        ...
        {
            return LowerExpression(tupleExpression.Values[0], destination);
        }
    ...
    private IExpressionResult LowerReturn(MethodReturnExpression methodReturnExpression)
    {
        ...
        {
            LowerExpression(methodReturnExpression.MethodReturn.Expression, new Local(ReturnValueLocalName));
        }
    ...
    private IExpressionResult LowerMethodCall(MethodCallExpression e, IPlace? destination)
    {
        ...
        IFunctionReference functionReference;

        // we're directly calling a method pointer
        if (e.MethodCall.Method.ResolvedType is TypeChecker.InstantiatedClass { Signature.Id: var resolvedTypeId }
            && resolvedTypeId == DefId.MethodPointer)
        {
            functionReference = new MethodPointerFunctionReference(LowerExpression(e.MethodCall.Method, destination: null).ToOperand());
    ...
    private void CreateClosureObject(TypeChecker.InstantiatedFunction instantiatedFunction, IPlace destination)
    {
        ...
        CreateObject(
            new LoweredPointer(BoxedValueType(closureTypeReference)),
            ClassVariantName,
            [],
            destination);
    ...
    {
        var value = LowerExpression(operand, destination: null);
    ...
    private IExpressionResult LowerVariableDeclaration(VariableDeclarationExpression e)
    {
        ...
        if (!referencedInClosure)
        {
            ...
            LowerExpression(e.VariableDeclaration.Value, destination: new Local(variable.CompilerGivenName));
            return result;
        }

        LowerExpression(e.VariableDeclaration.Value, destination: new Field(new Field(new Deref(new Local(LocalsObjectLocalName)), "Value", ClassVariantName), FieldName: variableName, ClassVariantName));
    ...
    private IExpressionResult LowerBinaryExpression(BinaryOperatorExpression binaryOperatorExpression, IPlace? destination)
    {
        ...
        {
            var leftResult = LowerExpression(binaryOperatorExpression.BinaryOperator.Left.NotNull(), destination: null);
